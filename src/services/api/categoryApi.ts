...
export const categoryApi = {
  ...
  create: (category: Omit<Category, "id">) =>
    apiService.post<Category>("product-categories", category),
  update: (id: number, category: Partial<Category>) =>
    apiService.put<Category>(`product-categories/${id}`, category),
  delete: (id: number) => apiService.delete<void>(`product-categories/${id}`),
  search: (query: string) =>
    apiService.get<Category[]>(
      `product-categories/search?query=${encodeURIComponent(query)}`
    ),
};
